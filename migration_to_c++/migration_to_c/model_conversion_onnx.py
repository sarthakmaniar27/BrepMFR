...
Usage (from repo root):
    ...
    python migration_to_c/model_conversion_onnx.py ^
        --checkpoint results/model_to_onnx/last.ckpt

Outputs:
    migration_to_c/exported/brepmfr_no_a2.onnx
    migration_to_c/exported/model_config.json
    migration_to_c/exported/label_map.json
...
for _anc in _THIS.parents:
    ...
    # Fallback: assume script is one level inside repo
...
# ═══════════════════════════════════════════════════════════════════
# Label map (same as run_pyg_inference.py)
# ═══════════════════════════════════════════════════════════════════
LABEL_MAP = {
    0: "Stock",
...
class BrepMFRONNXWrapper(nn.Module):
    ...
    Hardcodes the ``no_a2`` inference path (no d2_distance / angle_distance).
    ...
    re-packs them into the dict the encoder expects and runs:
        encoder → strip VNode → attention fusion → classifier → logits
    """
...
def load_brepseg(ckpt_path: Path, device: str = "cpu") -> Tuple[BrepSeg, Namespace]:
    ...
    args.max_nodes_for_a3 = None  # No cap for export — we want full A3

    # Handle class weights path that may not exist
...
def create_dummy_inputs(
    ...
    # Self-distance = 1 (after +1 shift in collator, 0 means padding)
    for i in range(N):
        spatial_pos[0, i, i] = 0  # will be shifted +1 during pad
...
def run_pytorch_checks(
    ...
    for N, E in test_sizes:
        ...
        print(f"    ℹ️  Predicted {len(unique_preds)} unique classes: {unique_preds.tolist()[:10]}")
...
def validate_onnx_vs_pytorch(
    ...
    for N, E, label in test_configs:
        ...
        if pt_logits.shape != ort_logits.shape:
            print(f"    ❌ Shape mismatch: PyTorch={pt_logits.shape}, ORT={ort_logits.shape}")
            all_passed = False
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Export BrepMFR to ONNX and validate parity",
    ...
    parser.add_argument(
        ...
        help="Directory for exported files (default: migration_to_c/exported/)",
    )
    parser.add_argument(
    ...
    if args.output_dir:
        ...
        output_dir = _THIS.parent / "exported"

    onnx_path = output_dir / "brepmfr_no_a2.onnx"
    ...
    num_classes = int(getattr(model_args, "num_classes", 25))
    ...
    if not args.skip_validation:
        ...
        if validation_passed:
            print("\n" + "=" * 60)
            print("✅ ALL CHECKS PASSED — ONNX model is ready for C++ deployment")
            print("=" * 60)
        else:
            print("\n" + "=" * 60)
