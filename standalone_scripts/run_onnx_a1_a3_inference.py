#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run the exported A1+A3 ONNX model (Thread + Text, 3 classes) on a PyG dataset.
...
This script is tailored for the A1+A3 "no_a2" profile. It passes all 16 required inputs to ONNX.
"""
...
FLOAT32_INPUTS = {"node_data", "face_area", "attn_bias", "edge_data", "edge_len", "edge_ang"}
INT64_INPUTS = {"face_type", "face_loop", "in_degree", "edge_type", "edge_conv", "edge_index", "spatial_pos", "edge_path"}
BOOL_INPUTS = {"padding_mask", "edge_padding_mask"}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the exported A1+A3 ONNX model (Thread + Text, 3 classes) on a "
    ...
    parser.add_argument(
        "--onnx",
        type=Path,
        default=Path("migration_to_c++/migration_to_c/no_a2_72k_epoch50_onnx/brepmfr_a1_a3.onnx"),
        help="Path to the exported A1+A3 ONNX model.",
    )
    parser.add_argument(
        "--label-map",
        type=Path,
...
def ensure_a1_a3_graph(graph: Any, graph_path: Path) -> None:
    """Validate that a PyG graph has the A1+A3 structural tensors required by
    the no_a2 ONNX model.  Unlike the lite validator, this *requires* that
    spatial_pos (A1) and edge_path (A3) are present."""
    # --- Basic node features that every BrepMFR graph must have ---
    required_node = ("node_data", "face_area", "face_type", "face_loop", "node_degree")
    # --- Edge features needed by the A1+A3 encoder ---
    required_edge = ("edge_data", "edge_index", "edge_type", "edge_len", "edge_ang", "edge_conv")
    # --- Structural proximity tensors (A1 and A3) ---
    required_struct = ("spatial_pos", "edge_path")

    missing = [
        name
        for name in (*required_node, *required_edge, *required_struct)
        if not hasattr(graph, name) or getattr(graph, name) is None
    ]
    if missing:
        raise ValueError(
            f"{graph_path.name}: missing tensors for A1+A3 profile: {missing}. "
            "Make sure the graph was created with --inference_profile no_a2."
        )
...
def make_a1_a3_batch(graph: Any) -> dict[str, torch.Tensor]:
    """Build the 16-tensor ONNX feed dict for a single A1+A3 PyG graph.

    This mirrors the production collator (data/collator.py) for batch_size=1,
    producing all the tensors the A1+A3 ONNX wrapper expects:
        Node:  node_data, face_area, face_type, face_loop, in_degree
        Edge:  edge_data, edge_type, edge_len, edge_ang, edge_conv, edge_index
        Struct: attn_bias, spatial_pos, edge_path
        Masks: padding_mask, edge_padding_mask
    """
    # --- Node features ---
    ...
    n_edges = int(edge_data.size(0))
    edge_index = graph.edge_index  # [2, E]

    # Flat per-face and per-edge scalars
    flat_features: dict[str, torch.Tensor] = {
        ...
        "edge_ang": graph.edge_ang,
        "edge_conv": graph.edge_conv,
    }
    ...
    # --- A1: spatial_pos [N, N] -> [1, N, N] ---
    spatial_pos = graph.spatial_pos.to(torch.int64)
    if spatial_pos.ndim == 2:
        spatial_pos = spatial_pos  # already [N, N]
    # The collator adds +1 to spatial_pos; replicate that here
    spatial_pos = spatial_pos + 1

    # --- A3: edge_path [N, N, K] -> [1, N, N, 16], pad/clamp to exactly K=16 ---
    # The ONNX model was exported with K=16 as a fixed dimension, so we must
    # match that exactly.  Pad with -1 (the sentinel for "no edge") if shorter.
    ONNX_K = 16
    edge_path = graph.edge_path.to(torch.int64)
    K = edge_path.size(-1)
    if K > ONNX_K:
        edge_path = edge_path[:, :, :ONNX_K]
    elif K < ONNX_K:
        pad_size = ONNX_K - K
        # Pad the last dimension with -1 (sentinel for empty path slots)
        edge_path = torch.nn.functional.pad(edge_path, (0, pad_size), value=-1)
    ...
    return {
        ...
        **flat_features,
        ...
        "attn_bias": attn_bias.unsqueeze(0),              # [1, N+1, N+1]
        "spatial_pos": spatial_pos.unsqueeze(0),           # [1, N, N]
        "edge_path": edge_path.unsqueeze(0),               # [1, N, N, K]
        "padding_mask": torch.zeros(1, n_faces, dtype=torch.bool),
        "edge_padding_mask": torch.zeros(1, n_edges, dtype=torch.bool),
    }
...
def batch_to_ort_feed(
    ...
    if missing:
        raise ValueError(f"A1+A3 batch did not produce required ONNX inputs: {missing}")
...
def main() -> int:
    ...
    missing_inputs = sorted(A1_A3_REQUIRED_INPUTS - input_names)
    unsupported_inputs = sorted(input_names - A1_A3_REQUIRED_INPUTS)
    if missing_inputs or unsupported_inputs:
        raise RuntimeError(
            "Unexpected ONNX input contract. "
            f"Missing A1+A3 inputs={missing_inputs}; unsupported inputs={unsupported_inputs}."
        )
